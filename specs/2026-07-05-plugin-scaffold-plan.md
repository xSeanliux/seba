...
**So: bake `${CLAUDE_PLUGIN_DATA}` straight into the SKILL.md command text.** It's
substituted to a literal absolute path at load time, before Claude runs anything —
no reliance on whether the var is exported into the model's ad-hoc Bash (it is
only guaranteed for hook/MCP subprocesses, not the model's Bash). The command in
the skill reads: `uv run --project "${CLAUDE_PLUGIN_DATA}" seba …`.
...
(unless `--keep-data`). That's fine — it only holds the venv, which `uv sync`
rebuilds. The learner's `$SEBA_DATA_DIR` (`~/seba-data`) is a separate user-level
dir and is never touched.
